...
| パラメータ名 | 値 | 説明 |
|------------|---|------|
| `<DBサーバーのホスト名>` | `<記入する>` | DBサーバーのFQDN（例：`az2-db.ex.local`） |
...
| パラメータ名 | 値 | 説明 |
|------------|---|------|
| `<SMTPサーバーのホスト名>` | `<記入する>` | SMTPサーバーのFQDN（例：`az1-smtp.ex.local`） |
...
| `<旧送信元メールアドレス>` | `info@mail.example.net` | application.yml に記載されている旧送信元 |

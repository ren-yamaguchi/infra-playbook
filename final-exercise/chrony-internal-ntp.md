...
| `<対象サーバーのホスト名>` | `<記入する>` | 設定対象のホスト名（NTPサーバー：例 `ntp-server`／クライアント：例 `web-az1`） |
| `<Primary DNSのIP>` | `<記入する>` | 内部DNSプライマリ（AZ2のAPサーバー）のIP |
| `<Secondary DNSのIP>` | `<記入する>` | 内部DNSセカンダリ（AZ4のAPサーバー）のIP |
| `<NTPサーバーのFQDN>` | `<記入する>` | NTPサーバーのFQDN（例：`db.ex.local`） |
...
### Step 1：システム設定（タイムゾーン・ホスト名・名前解決）【実施対象：NTPサーバー／NTPクライアント】

**目的：** タイムゾーン，ホスト名，内部DNSの名前解決先を設定する

> **補足：** 本Stepは NTPサーバーおよび全NTPクライアントで実施する．既に同設定が完了しているサーバー（前段の手順書で実施済）はスキップ可．
...
# ホスト名を設定（NTPサーバーまたは対象クライアントのホスト名）
hostnamectl set-hostname <対象サーバーのホスト名>
...
### Step 3：chronydのインストール確認【実施対象：NTPサーバー】
...
- **`chrony` が表示された場合（インストール済み）** → Step 4へ進む
...
### Step 4：chronydの自動起動設定と起動【実施対象：NTPサーバー】
...
### Step 5：設定ファイルのバックアップ【実施対象：NTPサーバー】
...
### Step 9：chronydの再起動【実施対象：NTPサーバー】
...
### Step 10：123番ポートでLISTENしているか確認【実施対象：NTPサーバー】
...
### Step 11：上位NTPサーバーとの同期確認【実施対象：NTPサーバー】
...
### Step 13：chronydのインストール確認【実施対象：NTPクライアント】

```bash
dnf list installed | grep chrony
```

- **`chrony` が表示された場合（インストール済み）** → Step 14へ進む
- **何も表示されない場合（未インストール）** → 以下を実行：
...
### Step 14：chronydの自動起動設定と起動【実施対象：NTPクライアント】
...
### Step 15：設定ファイルのバックアップ【実施対象：NTPクライアント】
...
### Step 17：設定ファイルの編集【実施対象：NTPクライアント】
...
### Step 18：追記内容の確認【実施対象：NTPクライアント】
...
### Step 19：chronydの再起動と同期確認【実施対象：NTPクライアント】
...
### 8-3. systemd-resolvedのDNS設定削除【実施対象：NTPサーバー／NTPクライアント】
...
### 8-4. 完了確認【実施対象：NTPサーバー／NTPクライアント】

...
> | v1.3 | 2026-06-21 | プレースホルダー命名の手順書内揺れを解消．実構成（AP1：AZ2／AP2：AZ4 の2台）に合わせて `<APサーバーのFQDN>` `<APサーバーのプライベートIP>`（単数形）を `<APサーバー1のFQDN>` `<APサーバー2のFQDN>` `<APサーバー1のプライベートIP>` `<APサーバー2のプライベートIP>`（複数形）に統一．対象：§3-3 パラメータ表，Step 1 注意書き，Step 2 事前確認，§6 トラブルシュート確認⑤，付録B 設定ファイル解説．付録Bのupstreamサンプルも2台構成に修正し，`ip_hash` の解説文も「将来の冗長化を見据えて1台」→「実構成2台」に書き換え．Step 4本体は元から2台upstream構成だったため変更なし． |
...
| `<APサーバー1のFQDN>` | `<記入する>` | プロキシ転送先のAPサーバー1（AZ2）のFQDN（例：`<任意の名前>-ap1.ex.local`） |
| `<APサーバー2のFQDN>` | `<記入する>` | プロキシ転送先のAPサーバー2（AZ4）のFQDN（例：`<任意の名前>-ap2.ex.local`） |
| `<APサーバー1のプライベートIP>` | `<記入する>` | APサーバー1のプライベートIP（FQDNが使えない場合の代替） |
| `<APサーバー2のプライベートIP>` | `<記入する>` | APサーバー2のプライベートIP（FQDNが使えない場合の代替） |
...
> **注意：** 本ステップは `nsd-private-redundancy.md` で内部DNSが構築されていることが前提．Step 2の `<APサーバー1のFQDN>` ／ `<APサーバー2のFQDN>` の名前解決も本Stepで設定したDNSを介して行われる．
...
```bash
# APサーバー1（AZ2）の名前解決確認
getent hosts <APサーバー1のFQDN>
# → IPアドレスが表示されれば成功

# APサーバー2（AZ4）の名前解決確認
getent hosts <APサーバー2のFQDN>
# → IPアドレスが表示されれば成功

# APサーバー1の8080番ポートへの疎通確認
curl -I http://<APサーバー1のFQDN>:8080
# → HTTP/1.1 200 など，APサーバーアプリケーションのレスポンスが返れば成功

# APサーバー2の8080番ポートへの疎通確認
curl -I http://<APサーバー2のFQDN>:8080
# → HTTP/1.1 200 など，APサーバーアプリケーションのレスポンスが返れば成功
```
...
2. APサーバーの8080番ポートへの疎通を確認する（AP1／AP2両方）．
...
- `ip_hash`：クライアントIPに基づいて常に同じAPサーバーに転送する（セッション固定，スティッキーセッション）．
- `server <APサーバー1のFQDN>:8080;` / `server <APサーバー2のFQDN>:8080;`：転送先APサーバー（AZ2／AZ4の2台）のFQDNとポート．

...
> | v1.3 | 2026-06-20 | 内部DNS（`nsd-private-redundancy.md`）の名前解決設定漏れに対応．新Step 1「システム設定（タイムゾーン・ホスト名・名前解決）」を追加し，後続Stepを2〜7に繰り上げ．パラメータ表に `<Primary DNSのIP>` `<Secondary DNSのIP>` 追加．ロールバック手順に「systemd-resolvedのDNS設定削除」を追加． |
...
### Step 1：システム設定（タイムゾーン・ホスト名・名前解決）【実施対象：DBサーバー】
...
# ホスト名を設定
hostnamectl set-hostname <DBサーバーのホスト名>
...
### Step 2：PostgreSQL 15 のインストール【実施対象：DBサーバー】
...
### Step 4：データベースクラスタの初期化（initdb）【実施対象：DBサーバー】
...
### Step 5：listen_addresses の設定【実施対象：DBサーバー】
...
### Step 6：PostgreSQLサービスの起動・自動起動設定【実施対象：DBサーバー】
...
#### エラー①：`postgresql.service: Failed to start`
...
> **補足：** `postgresql-setup --initdb` ラッパーを使えば，パーミッション・所有者は自動で正しく設定される．直接 `initdb` を実行した場合は手動確認が必要．
...
### 8-4. systemd-resolvedのDNS設定削除【実施対象：DBサーバー】
...
### 8-5. 完全リカバリ：AMIスナップショットからの復元（任意）【実施対象：AWSコンソール】
...
```bash
systemctl status postgresql.service 2>&1 | head -3
...
| `postgresql-setup --initdb` | PostgreSQLクラスタの初期化（AL2023のラッパー）．標準パス `/var/lib/pgsql/data` に作成． |
| `sudo -u postgres initdb -D <PATH>` | postgresユーザーとして直接initdbを実行． |
| `systemctl enable --now <サービス>` | サービスを起動し，自動起動を有効化． |
| `systemctl cat <サービス>` | unit ファイルの内容を表示． |
| `sudo -u postgres psql` | postgresユーザーとして psql を起動． |
...
**`/usr/lib/systemd/system/postgresql.service`（参考：パッケージ標準）**
...
- `postgresql-setup --initdb` ラッパーが `/var/lib/pgsql/data` に自動作成
- systemd unit ファイル（`/usr/lib/systemd/system/postgresql.service`）が `Environment=PGDATA=/var/lib/pgsql/data` を参照

...
│         <ZabbixサーバーのプライベートIP>/32 scram-sha-256  │
│                                                          │
└──────────────────────────────────────────────────────────┘
```
...
- [ ] `pg_hba.conf` に「`<ZabbixサーバーのプライベートIP>/32` から `<Zabbix用DBユーザー名>` で `<Zabbix用DB名>` へ scram-sha-256認証で接続許可」のエントリが追加されている
...
host    <Zabbix用DB名>    <Zabbix用DBユーザー名>    <ZabbixサーバーのプライベートIP>/32    scram-sha-256
```
...
- **md5認証と scram-sha-256 の違い**
  ...
  - 既存環境のパスワード認証方式が `md5` の場合の `scram-sha-256` への移行方法：
